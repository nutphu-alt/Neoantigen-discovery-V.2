...
workflow {
    ...
    // Mode 3: Neoantigen analysis
        ...
        collapse_deepsomatics_vcf_ch = deepsomatic.out.map { type, id, reads -> 
            def new_type
            if (type in ['tumor_rna', 'tumor_dna']) {
                new_type = 'tumor'
            } else if (type == 'normal_dna') {
        ...
        isec_deepsomatic = intersect_deepsomatic(tumor_deepsomatics_vcf_ch, normal_deepsomatics_vcf_ch)
