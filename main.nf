...
params.hisat2_index             = params.hisat2_index ?: "results/reference_index/hisat2/*"
...
include { hisat2_index }            from './modules/hisat2/index_building/hisat2_build_index.nf'
include { hisat2_align_reads }      from './modules/hisat2/align/hisat2_align_reads.nf'
...
include { somaticsniper }           from './modules/call_variants/somaticsniper/somaticsniper.nf'
...
include { intersec_somaticsniper }  from './modules/intersect_vcf/intersect_somaticsniper/isec_somaticsniper.nf'
...
workflow {
    ...
    fastqc_trimmed = fastqc_trimmed_reads(trim_reads.out)
    ...
    if (params.star_build_new_index) {
       log.info "🧬 Building new STAR index from reference and GTF"
        ...
        log.info "✅ Using existing STAR index: ${star_index}"
        star_index_files = Channel.fromPath("${star_index}*").collect()
    }
    ...
    if (params.bwa_build_new_index) {
       log.info "🧬 Building new BWA index from reference"
        ...
        log.info "✅ Using existing BWA index: ${bwa_index}"
        bwa_index_files = Channel.fromPath("${bwa_index}*").collect()
    }
    ...
    collapse_bcftools_vcf_ch = bcftools.out { type, id, reads -> 
        def new_type
    ...
    collapse_gatk_vcf_ch = gatk.out { type, id, reads -> 
        def new_type
        if (type in ['tumor_rna', 'tumor_dna']) {
            new_type = 'tumor'
        } else if (type == 'normal_dna') {
    ...
    collapse_deepsomatics_vcf_ch = deepsomatics.out { type, id, reads -> 
        def new_type
        if (type in ['tumor_rna', 'tumor_dna']) {
            new_type = 'tumor'
        } else if (type == 'normal_dna') {
    ...
    intersec_all_tools_ch = Channel.merge(intersec_bcftools.out, intersec_gatk.out, intersec_deepsomatics.out)
